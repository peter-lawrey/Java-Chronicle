...
import vanilla.java.chronicle.StopCharTesters;
...
import static vanilla.java.chronicle.tools.ChronicleTest.deleteOnExit;
...
public class AppendParseTest {
    ...
    private void assertText(Excerpt<IndexedChronicle> excerpt, int fid, String text) {
        assertEquals(fid, excerpt.parseLong());
        assertEquals(text, excerpt.parseEnum(String.class, CTRL_A));
    }

    private void assertText(Excerpt<IndexedChronicle> excerpt, int fid, Enum value) {
        assertEquals(fid, excerpt.parseLong());
        assertEquals(value, excerpt.parseEnum(value.getClass(), CTRL_A));
    }

    private void assertNum(Excerpt<IndexedChronicle> excerpt, int fid, long value) {
        assertEquals(fid, excerpt.parseLong());
        assertEquals(value, excerpt.parseLong());
    }

    private void assertNum(Excerpt<IndexedChronicle> excerpt, int fid, double value) {
        assertEquals(fid, excerpt.parseLong());
        assertEquals(value, excerpt.parseDouble());
    }

    private static void appendNum(Excerpt<IndexedChronicle> excerpt, int fid, long value) {
        excerpt.append(fid).append('=').append(value).append((char) 1);
    }

    private static void appendNum(Excerpt<IndexedChronicle> excerpt, int fid, double value, int precision) {
        excerpt.append(fid).append('=').append(value, precision).append((char) 1);
    }

    private static void appendText(Excerpt<IndexedChronicle> excerpt, int fid, String chars) {
        excerpt.append(fid).append('=').append(chars).append((char) 1);
    }

    private static void appendText(Excerpt<IndexedChronicle> excerpt, int fid, Enum value) {
        excerpt.append(fid).append('=').append(value).append((char) 1);
    }
}
